...
test('Lautstaerke wirkt ohne Neustart — kein FFmpeg, kein yt-dlp', async () => {
    const guildId = 'cache-7';
    const log = createLog();
    ctx.setSpawn(makeFakeSpawn(log, { audioDelayMs: 30 }));
    const queue = freshQueue(log, guildId);
    queue.tracks.push({ ...TRACK });
    ...
    const before = log.find('yt-dlp.stream').length;
    const resourceBefore = queue._resource;

    queue.volume = 0.35;
    ctx.applyAudioSettings(guildId, { volumeOnly: true });
    await sleep(600);   // laenger als die 400ms-Buendelung eines Neustarts

    assert.strictEqual(log.find('yt-dlp.stream').length, before, 'kein neuer Download');
    assert.strictEqual(queue._resource, resourceBefore,
        'derselbe Audio-Strom — ein Neustart haette ihn ersetzt');
    assert.ok(Math.abs(queue._resource.volume.volume - 0.35) < 0.01,
        `Lautstaerke muss am laufenden Strom ankommen, war ${queue._resource.volume?.volume}`);
    cleanup(guildId);
});
